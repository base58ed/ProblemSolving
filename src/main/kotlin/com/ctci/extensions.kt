...
//Int related
fun random(n: Int): Int = Math.random().toInt() * n

fun random(min: Int, max: Int) = random(max - min + 1) + min

fun randomMatrix(m: Int, n: Int, min: Int, max: Int): Array<IntArray> = Array(m) { IntArray(n) { random(min, max) } }


//Boolean related
fun randomBoolean(): Boolean = random(0, 1) == 0

fun randomBoolean(percent: Int): Boolean = random(1, 100) <= percent

fun randomBooleanMatrix(m: Int, n: Int): Array<BooleanArray> = Array(m) { BooleanArray(n) { randomBoolean() } }


//IntArray related
fun IntArray.random(n: Int, min: Int, max: Int) = IntArray(n) { random(min, max) }

fun IntArray.prettyPrint() = println(java.util.Arrays.toString(this))


//matrix related
fun Array<IntArray>.prettyPrint() {
	for (i in 0..this.count()-1) {
		for (j in 0..this[i].count()-1) {
			if (this[i][j] in -9..9) println(" ")
			if (this[i][j] in -99..99) println(" ")
			if (this[i][j] >= 0) println(" ")
			println(" ${this[i][j]}")
		}
		println()
	}
...
fun Array<BooleanArray>.prettyPrint() {
	for (i in 0..this.count()-1) {
		for (j in 0..this[i].count()-1) {
			if (this[i][j]) println("1")
			else println("0")
		}
		println()
	}
...
fun <K, V> MutableMap<K, MutableList<V>>.put(key: K, value: V) {
	if (this.contains(key = key)) {
		...
		this.put(key = key, value = arrayListOf(value))
	}
}

fun <K, V> MutableMap<K, MutableList<V>>.containsKeyValue(key: K, value: V): Boolean {
	this[key]?.let {
...
fun <K, V> Map<K, List<V>>.prettyPrint() {
...
//String related
fun Array<String>.toString(): String = this.joinToString()

fun longTextBlob(): String {
	val book = """
...
fun longTextBlobArray(): Array<String> = longTextBlob().split(" ").toTypedArray()

fun words(): Array<String> = arrayOf( "the", "of", "and", "a", "to", "in", "is", "be",
